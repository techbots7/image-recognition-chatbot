        ...
        try {
            ...
            for (const path of paths) {
                try {
                    ...
                    // First check if the model.json file exists
                    const modelJsonResponse = await fetch(path);
                    if (!modelJsonResponse.ok) {
                        throw new Error(`Failed to fetch model.json from ${path}: ${modelJsonResponse.statusText}`);
                    }
                    
                    // Parse the model.json to check its structure
                    const modelJson = await modelJsonResponse.json();
                    console.log('Model JSON structure:', {
                        kerasVersion: modelJson.keras_version,
                    ...
                    if (error.message.includes('producer')) {
                        console.error('Producer error details:', {
                            error: error.message,
                            stack: error.stack,
                            path: path
                        });
